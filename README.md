...
Веб-сервис на **Flask** + **PaddleOCR**: загрузка изображения, камера в браузере, IP-камера (URL кадра) и локальная камера на стороне сервера (OpenCV). Результат — строка номера в формате **одна буква (допустимые на РФ) + три цифры + две буквы**, опционально с **регионом** (2–3 цифры).
...
| Поле | Описание |
|------|----------|
| `plate` | Полный номер: основная часть + регион (если есть) |
...
- Строка OCR нормализуется: латиница в похожие кириллические буквы (`LATIN_TO_CYRILLIC`), частые путаницы цифр/букв (`DIGIT_TO_LETTER`, `LETTER_TO_DIGIT`).
